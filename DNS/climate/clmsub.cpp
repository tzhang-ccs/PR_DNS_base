...
static void nb_state(COMPONENT* c,double* coords,PHASE_FIELD* field,int index,int dim,PARAMS* eqn_params)
{
	double q0, T0;
	...
	field->temperature[index] = T0 - 0.608*T0*0.001*(field->vapor[index] - q0);
	...
	switch (dim)
	{
	    case 2:
			...
			for (j = 0; j < gmax[1]; j++)
			{
				...
				E = 16.0/sqrt(0.5*M_PI)*u0*u0*pow(wn,4)/pow(w0,5)* exp(-2.0*wn*wn/(w0*w0)); 
